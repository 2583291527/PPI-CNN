...
model=dynamic_model(36,32,64)
model.load_state_dict(torch.load(r"C:\Users\25832\Desktop\CNN-GRU\save_modelsss\2net.pth"))
model.eval()

def load_all_data(exp_settings):
    def load_data(mode):
        dataset = ProtPep_dataset(ws=exp_settings['window_size'], mode=mode)
        return DataLoader(dataset, shuffle=False, batch_size=exp_settings['batch_size'])
...
data = pd.read_csv(r"C:\Users\25832\Desktop\CNN-GRU\test_36_set.csv",header=None)
...
def testcalc_metrics(iter_dict):
    scores = {}
    #print('1真实值',iter_dict['val_labels'],len(iter_dict['val_labels']))
    #print('2预测值',iter_dict['val_outputs'],len(iter_dict['val_outputs']))
    ...
    scores['mcc'] = mcc_numerator / np.sqrt(mcc_denominator)
...
for (images,labels) in data['test']:#进行测试
    inputs = Variable(images.clone().detach()).to(device)
    ...
    out_max = torch.argmax(out_max, dim=1)#输出是哪一类
    #print('标签',out_max)
...
print('5',testcalc_metrics(iter_dict))